...
let rec concat_length (xs ys : list int)
  : Lemma (length (xs @ ys) == length xs + length ys) =
  match xs with
  | [] -> ()
  | x :: xs' -> concat_length xs' ys
...
let rec to_list_length (t:bst) : Lemma (length (to_list t) == size t) =
  ...
  | N (l, x, r) -> (
    to_list_length l;
    to_list_length r;
    concat_length (to_list l) [x];
    concat_length ((to_list l) @ [x]) (to_list r)
  )
